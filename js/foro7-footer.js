...
function track(){var s=slug();if(!s)return;fetch(SB+'/rest/v1/eventos?slug=eq.'+s+'&select=id&limit=1',{headers:H}).then(function(r){return r.json();}).then(function(d){if(d[0]&&d[0].id)fetch(SB+'/rest/v1/visitas',{method:'POST',headers:Object.assign({},H,{'Prefer':'return=minimal'}),body:JSON.stringify({evento_id:d[0].id,pagina:'click-foro7',session_id:sid()})});}).catch(function(){});}
function inject(){var f=document.querySelector('footer,.footer,[class*="footer"]');if(!f||f.querySelector('.foro7-web-link'))return;var p=pick();var a=document.createElement('a');a.href=p.url;a.target='_blank';a.rel='noopener';a.textContent='\uD83C\uDF10 '+p.label;a.style.cssText='color:inherit;opacity:.8;text-decoration:none;font-size:.88em;';a.addEventListener('click',track);var el=document.createElement('p');el.className='foro7-web-link';el.style.marginTop='6px';el.appendChild(a);f.appendChild(el);}
